...
def _tools_to_openai(tools: Sequence[tools_.ToolLike]) -> list[dict[str, Any]]:
...
async def _file_part_to_openai(part: messages_.FilePart) -> dict[str, Any]:
    ...
    * anything else → ``ValueError``
    ...
    if mt.startswith("image/"):
        ...
        url = media.data.data_to_data_url(data, media_type)
        return {"type": "image_url", "image_url": {"url": url}}

    if mt.startswith("audio/"):
        # OpenAI input_audio requires raw base-64 — download http(s) URLs.
        if isinstance(data, str) and media.data.is_downloadable_url(data):
            downloaded, _ = await media.download.download(data)
            data = downloaded
        fmt = mt.split("/", 1)[1] if "/" in mt else mt
        b64 = media.data.data_to_base64(data)
    ...
    if mt == "application/pdf":
        ...
        if isinstance(data, str) and media.data.is_downloadable_url(data):
            downloaded, _ = await media.download.download(data)
            data = downloaded
        data_url = media.data.data_to_data_url(data, mt)
...
class OpenAIModel(llm_.LanguageModel):
        ...
        async for chunk in stream:
            ...
            if reasoning_value:
                if not reasoning_started:
                    ...
                    yield llm_.ReasoningStart(block_id="reasoning")
                yield llm_.ReasoningDelta(block_id="reasoning", delta=reasoning_value)

            if delta.content:
                # Close reasoning block when text starts (reasoning precedes text)
                if reasoning_started:
                    yield llm_.ReasoningEnd(block_id="reasoning")
                    reasoning_started = False

                if not text_started:
                    text_started = True
                    yield llm_.TextStart(block_id="text")
                yield llm_.TextDelta(block_id="text", delta=delta.content)
            ...
            if choice.finish_reason is not None:
                ...
                if reasoning_started:
                    yield llm_.ReasoningEnd(block_id="reasoning")
                if text_started:
                    yield llm_.TextEnd(block_id="text")
                for tc in tool_calls.values():
                    if tc["started"] and tc["id"]:
                        yield llm_.ToolEnd(tool_call_id=tc["id"])
