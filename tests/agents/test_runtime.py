...
async def test_agent_tool_then_text() -> None:
    ...
    async with my_agent.run(
        model=MOCK_MODEL, messages=[ai.user_message("Double 5")]
    ) as stream:
        msgs = await collect_messages(stream)
...
async def test_agent_parallel_tools() -> None:
    ...
    async with my_agent.run(
        model=MOCK_MODEL, messages=[ai.user_message("Double 3 and 7")]
    ) as stream:
        msgs = await collect_messages(stream)
    assert llm.call_count == 2
...
async def test_agent_multi_turn() -> None:
    ...
    async with my_agent.run(
        model=MOCK_MODEL, messages=[ai.user_message("Concat then double")]
    ) as stream:
